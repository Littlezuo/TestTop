package com.topfine.malltest.base.net.callback;

import com.google.gson.stream.JsonReader;
import com.lzy.okgo.callback.AbsCallback;
import com.lzy.okgo.request.BaseRequest;
import com.topfine.malltest.base.net.Convert;
import com.topfine.malltest.base.net.basemodel.Errors;
import com.topfine.malltest.base.net.basemodel.LzyResponse;
import com.topfine.malltest.base.net.basemodel.SimpleResponse;
...
public abstract class JsonCallback<T> extends AbsCallback<T> {
    ...
    public T convertSuccess(Response response) throws Exception {
        ...
        if (rawType == Void.class) {
            ...
            if ("OK".equals(lzyResponse.status)) {
                return (T) lzyResponse;
            } else if("ERR".equals(lzyResponse.status)){
                int code = 0;
                String errorStr = "";
                    Errors error = (Errors) lzyResponse.errors.get(0);  //取第一个
                    code = error.errid;
                    errorStr = error.errors;

                    if (code == 104) {
                        //比如：用户授权信息无效，在此实现相应的逻辑，弹出对话或者跳转到其他页面等,该抛出错误，会在onError中回调。
                        throw new IllegalStateException(error.errors);
                    } else if (code == 105) {
                ...
                throw new IllegalStateException("返回数据格式错误!");
            }
        } else {
            response.close();
